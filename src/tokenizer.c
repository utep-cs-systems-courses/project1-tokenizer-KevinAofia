...
{
  if (space_char(*inStr)) { //if string starts with a space
    while(space_char(*inStr)) { //while spaces are present
      inStr++; //move by one address
      len--; //subtract from the original length count passed
    }
  }
  ...
  copy[i] = '\0'; //set next address value to terminating character
  return copy;
}
...
char **tokenize(char *str) {
  ...
  tokens[i] = '\0'; //put term character in last array
  //printf("Done\n");
  return tokens;
}
...
void free_tokens(char **tokens)
{
  int i = 0;
  while(tokens[i] != NULL) {
    free(tokens[i]); //free what second layer
    i++;
  }
  free(tokens); //free the top layer pointing at second layer
